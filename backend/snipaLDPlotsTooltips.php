...
if ($type == "annotation") {
	if ($element == "gene") {
		$anno = unserialize(file_get_contents($jobpath."/ldplot_genelist_annotations_phparray.txt"));
		print($anno[$name]['ANNOTATION']);
	}
	else if ($element == "regelement") {
		$anno = unserialize(file_get_contents($jobpath."/ldplot_regulatorylist_annotations_phparray.txt"));
		print($anno[$name]['ANNOTATION']);
	}
	else if ($element == "snp") {
		$anno = unserialize(file_get_contents($jobpath."/ldplot_snps_annotations_phparray.txt"));
