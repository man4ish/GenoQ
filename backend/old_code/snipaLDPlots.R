...
locus.plot.classic <- function(querysnp, chr, snplist, genelist, highlightpos = c(), highlightmode = "show") {
  ...
  # highlight-modus: snps, die nicht hervorgehoben sind, transparenter machen
  if (length(highlightpos) > 0) { snplist[which(!(snplist$POS %in% highlightpos)),'COLOR'] <- rgb(col2rgb(snplist[which(!(snplist$POS %in% highlightpos)),'COLOR'])['red',]/255,col2rgb(snplist[which(!(snplist$POS %in% highlightpos)),'COLOR'])['green',]/255,col2rgb(snplist[which(!(snplist$POS %in% highlightpos)),'COLOR'])['blue',]/255,0.05) }
  ...
  # highlight-modus: snps, die nicht hervorgehoben sind, transparenter machen
  if (length(highlightpos) > 0) { snplist[which(!(snplist$POS %in% highlightpos)),'BORDERCOLOR'] <- rgb(col2rgb(snplist[which(!(snplist$POS %in% highlightpos)),'BORDERCOLOR'])['red',]/255,col2rgb(snplist[which(!(snplist$POS %in% highlightpos)),'BORDERCOLOR'])['green',]/255,col2rgb(snplist[which(!(snplist$POS %in% highlightpos)),'BORDERCOLOR'])['blue',]/255,0.1) }
  ...
  snplist[which(snplist$POS %in% highlightpos),'PRIORITY'] <- 12
  
  snplist <- arrange(snplist,PRIORITY)

  # falls nicht-hervorgehobene SNPs ausgeblendet werden sollen
  if ((length(highlightpos) > 0) && (highlightmode == "hide")) { snplist <- snplist[which(snplist$POS %in% c(highlightpos,snplist$POS[snplist['SNP'] == querysnp])),] } 
  ...
  mtext(expression("Linkage disequilibrium" ~ (r^2)), side=2, at=(range/2), line=2, cex=0.7)
...
locus.plot.highcharts <- function(querysnp, chr, snplist, genelist, regellist, highlightpos = c(), highlightmode = "show") {
# highlightmode options: hide | show
...
# highlight-modus: snps, die nicht hervorgehoben sind, transparenter machen
if (length(highlightpos) > 0) { snps[which(!(snps$x %in% highlightpos)),'color'] <- paste0("rgba(",col2rgb(snps[which(!(snps$x %in% highlightpos)),'color'])['red',],",",col2rgb(snps[which(!(snps$x %in% highlightpos)),'color'])['green',],",",col2rgb(snps[which(!(snps$x %in% highlightpos)),'color'])['blue',],",0.2)") }
...
# Rahmenfarbe
snps$lineColor <- "#444444"
snps$lineColor[snplist['MULTIPLE'] == 1] <- "#006611"
snps$lineColor[snplist['DISEASE'] == 1] <- "#003399"

# highlight-modus: snps, die nicht hervorgehoben sind, transparenter machen
if (length(highlightpos) > 0) { snps[which(!(snps$x %in% highlightpos)),'lineColor'] <- paste0("rgba(",col2rgb(snps[which(!(snps$x %in% highlightpos)),'lineColor'])['red',],",",col2rgb(snps[which(!(snps$x %in% highlightpos)),'lineColor'])['green',],",",col2rgb(snps[which(!(snps$x %in% highlightpos)),'lineColor'])['blue',],",0.2)") }
...
# falls highlight-modus: snps, die hervorgehoben werden sollen, größer plotten
if ((length(highlightpos) > 0) && (highlightmode == "show")) { snps[which(snps$x %in% highlightpos),'radius']  <- 7 }
...
for (grpcnt in c(1:length(groupname))) {
	...
	if ((length(highlightpos) > 0) && (highlightmode == "hide")) { snpstmp <- snpstmp[which(snpstmp$x %in% c(highlightpos,snps$x[snplist['SNP'] == querysnp])),] }
