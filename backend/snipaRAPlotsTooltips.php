...
if ($type == "annotation") {
	if ($element == "gene") {
		$anno = unserialize(file_get_contents($jobpath."/raplot_genelist_annotations_phparray.txt"));
		print($anno[$name]['ANNOTATION']);
	}
	else if ($element == "regelement") {
		$anno = unserialize(file_get_contents($jobpath."/raplot_regulatorylist_annotations_phparray.txt"));
		print($anno[$name]['ANNOTATION']);
	}
	else if ($element == "snp") {
		$anno = unserialize(file_get_contents($jobpath."/raplot_snps_annotations_phparray.txt"));
