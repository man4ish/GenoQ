...
if(ref==0)
{
    char titlefilename[2000];
    strcpy(titlefilename,jobdir);
    strcat(titlefilename,"/title.json");

    //cout << titlefilename<< endl;
    ofstream jsontitlefile(titlefilename);
    jsontitlefile<<"{"<<"\"text\":"<<"\"LDHeatmap "<<pop<<" vs Qatar Population\""<<","<<"\"value\":"<<"42542352345}"<<"\n";
    jsontitlefile.close();
    

    char static_title[500]; 
    strcpy(static_title,jobdir);
    strcat(static_title,"/title.sample");
    ofstream titlefile(static_title);
    //cout << static_title<<endl;
   ...
   char staticdatafile[500];
   strcpy(staticdatafile,jobdir);
   strcat(staticdatafile,"/data.sample");
   //cout << staticdatafile << endl;
   ofstream outfile;
   if(ref)
      ofstream outfile(staticdatafile, std::ofstream::app);
   else 
      ofstream outfile(staticdatafile);
   ...
   for (int i=1; i<(pos1.size()-1);i++)
   {
       for (int j=1; j<(pos1.size()-1);j++)
       {
               if(i<j)
               {
                 ...
                 if(i1[0]!=0)
                 {
                   //cout<<i1[0]<<"\t"<<i1[1]<<"\t"<<i2[0]<<"\t"<<i2[1]<<"\t"<<i3[0]<<"\t"<<i3[1]<<"\t"<<i4[0]<<"\t"<<i4[1]<<"\t\""<<color<<"\""<<endl;
  ...
  {
     char dataline[2000];
     strcpy(dataline,jobdir);
     strcat(dataline,"/data.line");
     //cout<<dataline<<endl;
     ofstream out(dataline);
