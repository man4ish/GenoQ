...
function showResults(randid) {
	...
	$('#plots-tabs-static').html('<img id="plot-static-image" src="tmpdata/'+randid+'/raplot_plot_static.png" />');
	
	$.ajax({url: 'tmpdata/'+randid+'/report.txt', async: false, dataType: "json"})
		.done(function(report) { 
			var reporthtml = "";
			reporthtml += "<p>The <a href='"+report['userinput']['originalfn']+"' style='color: rgb(228,0,58);' target='_blank'>uploaded data</a> contains "+report['userinput']['originalcnt']+" lines. The <a href='"+report['userinput']['filteredfn']+"' style='color: rgb(228,0,58);' target='_blank'>preprocessed input data</a> contains "+report['userinput']['filteredcnt']+" lines. </p>";
			reporthtml += "<p>Variants were annotated using the &quot;" + report['userinput']['genomerelease'] + "/" + report['userinput']['referenceset'] +  "&quot; variant set (&quot;" + report['userinput']['population'] +  "&quot; population). Functional annotations are based on &quot;" + report['userinput']['annotation'] +  "&quot;.</p>";
			reporthtml += "<p>The SNP " + report['userinput']['sentinelname'] + " was used as the sentinel variant. It is located on Chromosome " + report['snppos']['sentinelchr'] + " at position " + report['snppos']['sentinelpos'] + ".  Of your input data, <a href='"+report['snppos']['mappedfn']+"' style='color: rgb(228,0,58);' target='_blank'>" + report['snppos']['mappedcnt'] + " SNPs could be mapped</a> to genetic positions within a +/- 250 kb window around the sentinel variant (min/max position: "+report['snppos']['min']+"/"+report['snppos']['max']+")";
			sentinelname = report['userinput']['sentinelname'];
			if (report['snppos']['notmappedcnt'] > 0) {
				reporthtml += ", whereas <a href='"+report['snppos']['notmappedfn']+"' style='color: rgb(228,0,58);' target='_blank'>" + report['snppos']['notmappedcnt'] + " variants could not be mapped</a>";
			}
			reporthtml += ".</p>";
			...
			$('#plots-tabs-report').prepend(reporthtml); 
		});
	...
	if ( $("input[name=highcharts]:checked").val() == 1) {
		...
		tmpsnpinfo += '<span id="nosnpinfo">Click on a variant in the interactive plot. Detailed annotations will then be displayed in this section.</span>'; 
		...
		$.ajax(
			{
			  url : 'tmpdata/'+randid+'/raplot_plot_dynamic.js',
			  type: "GET",
			  dataType:"script",
			  cache: false,
			  async: false,
			  success: 
				function() {
				   ...
				   chartoptions.exporting.filename = "snipa_raplot_"+sentinelname+"_"+$("select#dataset-genomerelease").val()+"_"+$("select#dataset-referenceset").val()+"_"+$("select#dataset-population").val()+"_"+$("select#dataset-annotation").val();
...
$( window ).resize(function() {
	...
	if ($("#plot-static-image").length) {
		...
		$("#plot-static-image").attr("src",imgpath + "/raplot_plot_static_"+ closest + ".png");		
	}
});
	...
	$.ajax(
			{
			  ...
			  success: function(anno) { 
					...
					$("#plots-snpinfo-accordion h3 span.pinkspan").click(function() {return false;} ); // Verhindert das ein- und ausklappen bei den löschen und snpbin buttons
...
function showPlotAnnotationMenu(eventtype,snpname,snppos,snpchr,sentinelpos,genomerelease,referenceset,population,annotation) { 
	if (snpclickdefault == 0) {
		...
		$('#snipa-raplot-dynamic-snpdetails-menu').html('You can either get detailed annotations for this variant or copy it to SNiPA\'s clipboard.<br /><input type=\"checkbox\" id=\"snipa-raplot-dynamic-snpdetails-sameaction\" style=\"width: 10px;\" /> Use this as default action for this session.');  
		} 
