$(document).ready(function(){
	...
	$("#highlightmode_container").buttonset();
	...
	// Button für SNP pastebin bei Highlight-Variants
	if ($("#snpbinbox").is(":visible")) {
		$("#highlightvariants").after("<button id='highlightvariants_from_pastebin'>Paste from SNiPA's clipboard</button>");
		$("#highlightvariants_from_pastebin").button().click(function() {
			$.ajax({
				  url: "backend/snipaSnpBin.php",
				  type: "POST",
				  data: { task: "read" },
				  ...
				  success: function(data) {	
					$.each(data, function(i,v) {
						$("#highlightvariants").val($("#highlightvariants").val()+v.value+'\n');
					});
				  }
			  });
			}
				...
				$.ajax(
					{
					  ...
					  data: {	id: randid, 
								...
								highlightvariants: $("textarea#highlightvariants").val(),
								highlightmode: $("input[name=highlightmode]:checked").val(), 
